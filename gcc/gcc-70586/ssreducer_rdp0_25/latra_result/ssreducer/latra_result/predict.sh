...
gcc_4_9_0="$run_in_docker timeout -s 9 $TIMEOUTCC gcc-4.9.0"
...
if
  timeout -s 9 $TIMEOUTCC $clang_7_1_0 -Wfatal-errors -pedantic -Wall -Wsystem-headers -O0 -c $CFILE > out.txt 2>&1 \
    ...
    && $gcc_7_1_0 -Wfatal-errors -Wall -Wextra -Wsystem-headers -O0 $CFILE > outa.txt 2>&1 \
    &&
...
rm -f $MEM_SANITIZER_OUTPUT $TEMP_EXE
