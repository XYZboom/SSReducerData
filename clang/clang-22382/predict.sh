...
chmod 777 .
run_in_docker="docker run --rm -v $(realpath .):/tmp/clang-22382 -w /tmp/clang-22382 cnsun/perses:perses_part_54_name_clang_trunk"
clang_3_6_0="$run_in_docker clang-3.6.0 -w -c"
clang_7_1_0="$run_in_docker clang-7.1.0 -w -c"
gcc_7_1_0="$run_in_docker gcc-7.1.0 -c"
...
#if
#  timeout -s 9 $TIMEOUTCC $clang_7_1_0 -pedantic -Wall -Wsystem-headers -O0 -c $CFILE > out.txt 2>&1 \
...
#    && timeout -s 9 $TIMEOUTCC $gcc_7_1_0 -Wall -Wextra -Wsystem-headers -O0 $CFILE > outa.txt 2>&1 \
#    &&
...
# we do not exec, we only need a compiler crash, that's enough
