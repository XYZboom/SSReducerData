...
clang_3_7_0="$run_in_docker timeout -s 9 $TIMEOUTCC clang-3.7.0"
...
readonly GOODCC=("$gcc_4_8_0 -m32 -O0")
...
if
  timeout -s 9 $TIMEOUTCC $clang_7_1_0 -Wfatal-errors -pedantic -Wall -Wsystem-headers -O0 -c $CFILE > out.txt 2>&1 \
    ...
    && timeout -s 9 $TIMEOUTCC $gcc_7_1_0 -Wfatal-errors -Wall -Wextra -Wsystem-headers -O0 $CFILE > outa.txt 2>&1 \
    &&
...
($run_in_docker timeout -s 9 $TIMEOUTEXE ./$TEMP_EXE &> $MEM_SANITIZER_OUTPUT) &> /dev/null
if [[ $? != 0 ]]; then
  exit 1
fi
...
$run_in_docker rm $MEM_SANITIZER_OUTPUT $TEMP_EXE
